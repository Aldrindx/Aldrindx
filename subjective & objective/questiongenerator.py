...
def print_jsn(qa_list: List[Mapping[str, str]], show_answers: bool = True, data: str = "") -> None:
    ...
    for i in range(len(qa_list)):
        ...
        empty_list.append(f"{i + 1}) Q: {qa_list[i]['question'] + space+space} Sim_score: {nlp(data).similarity(nlp(qa_list[i]['question'])):.4f}")
        ...
        if type(answer) is list:
            ...
            if show_answers:
                empty_list.append(
                    ...
                    f"{space+space} Score: {torch.sigmoid(ans._evaluate_qa(ans._encode_qa(qa_list[i]['question'],answer[0]['answer']))):.4f}"
                    f"{space}{np.where(answer[0]['correct'] , '(correct)', '')}"                    
                )
                
                for j in range(1, len(answer)):
                    empty_list.append(
                        f"{space + '   '}{j + 1}. {answer[j]['answer']} "
                        f"{space+space} Score: {torch.sigmoid(ans._evaluate_qa(ans._encode_qa(qa_list[i]['question'],answer[j]['answer']))):.4f} "
